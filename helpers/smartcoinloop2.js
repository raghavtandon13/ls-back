...
async function sendToSmartcoin(users) {
    ...
    for (const user of users) {
        try {
            ...
            user.refArr.push({ name: "sm_19mar_shizu" });
            await user.save();
        } catch (error) {
    ...
    try {
        ...
        while (true) {
            ...
            const allUsers = await User.find({
                accounts: { $elemMatch: { name: "SmartCoin", resp_date: { $lte: new Date("2025-02-02") } } },
		"refArr.name": "RamFinNEWshizuSM",
                "refArr.name": { $ne: "sm_19mar_shizu" },
            })
                .skip(skip)
                .sort({ updatedAt: -1 })
                .limit(BATCH_SIZE);
