...
const partnerApiKeys = {
    ...
    vs99Cu06K1GB2xthm8j: "MoneyTap_TF_UAT", // UAT
    qP7XaJ39L5MTvKdyW2z: "MoneyTap_TF", // Prod
    // qP7XaJ3tocmKdyW2z: "LS_TF", // Prod
    vs65Cu06INTER1GB2qSdJejP: "DTMF",
    vs65Cu06aghR1GB2qxygeL: "Zype_LS",
};
...
router.post("/inject", checkLeadAuth, async function (req, res) {
    const { lead } = req.body;
    try {
        const [dbPromise, _] = await Promise.allSettled([addtoDB(lead), addtoCM(lead)]);
...
router.post("/inject2", async function (req, res) {
    ...
    const availableLenders = ["SmartCoin"];
    let lenders = availableLenders.filter((lender) => mainList.includes(lender));

    // Override Lenders here
    lenders.push("Zype");
    lenders.push("MPocket");
    lenders.push("SmartCoin");
    lenders.push("FatakPay");
    ...
    if (lenderStop) {
        ...
        lenders = lenders.filter((lender) => lender !== "RamFin");
    }
    ...
    if (lenders.includes("SmartCoin")) promises.push(smartcoinInject(lead));
    ...
    try {
        ...
        const smResult = lenders.includes("SmartCoin") ? results.shift() : null;
        const smRes = smResult
            ? smResult.status === "fulfilled"
                ? smResult.value
...
async function addtoDB(lead) {
    ...
    console.log(lead);
    console.log("incoming lead: ", lead.phone);
    let user = await User.findOne({ phone: lead.phone });
    let status = "";
    if (!user) {
        logToFile(`NEW : ${lead.phone}`);
        status = "success";
        newUser = new User({
            ...
            intent: lead.intent || false,
            otpVerified: lead.otpVerified || false,
        ...
        user.consent = lead.consent || "";
        user.consentIp = lead.consentIp || "";
        user.intent = lead.intent || false;
        user.otpVerified = lead.otpVerified || false;
        user.creditScore = lead.creditScore || "";
...
async function addtoCM(lead) {
    const cmRes = await axios.post(
        "https://credmantra.com/api/v1/leads/inject",
        { lead: lead, injectIfDup: false },
        { headers: { "x-api-key": "qP7XaJ3tocmKdyW2z", "Content-Type": "application/json" } },
    );
    console.log("CM Response: ", cmRes.data);
}
...
async function upwards2Inject(lead) {
    const apiUrl1 = "http://localhost:5000/api/v1/partner-api/upwards2/create";
    const comman = {
        consent: true,
        current_city: lead.city,
        current_pincode: lead.pincode.toString(),
...
async function paymeInject(lead) {
    ...
    while (retries > 0) {
        try {
            ...
            console.log("Retrying cibil req");
            retries--; // Decrement the retries counter
            if (retries === 0) {
                return "Max retries exceeded"; // Return an error after max retries
            }
        }
    }
    try {
        ...
        console.error("Error in limit request:", error);
    }
...
async function zypeInject(lead) {
    const zypeDedupeURL = "https://credmantra.com/api/v1/partner-api/zype/dedupe";
    const zypeOfferURL = "https://credmantra.com/api/v1/partner-api/zype/offer";
...
async function mpocketInject(lead) {
    ...
    let dedupeRes, leadRes, statusRes;

    try {
        const dedupeReq = { mobileNumber: lead.phone, email: lead.email };
        dedupeRes = await axios.post(dedupeURL, dedupeReq);
    } catch (error) {
        console.error("Dedupe API Error:", error.response?.data || error.message);
    }
    ...
    try {
        ...
        leadRes = await axios.post(leadURL, leadReq);
    } catch (error) {
        console.error("Lead API Error:", error.response?.data || error.message);
    }

    //try {
    //    const statusReq = { request_id: leadRes?.data?.data?.requestId };
    //    statusRes = await axios.post(statusURL, statusReq);
    //} catch (error) {
    //    console.error("Status API Error:", error.response?.data || error.message);
    //}

    // Log all API responses
    console.log("API Responses:", {
        dedupeResponse: dedupeRes?.data || null,
        leadResponse: leadRes?.data || null,
        statusResponse: statusRes?.data || null,
    });

    // Return the last non-error value
    if (statusRes && statusRes.data) return statusRes.data;
    if (leadRes && leadRes.data) return leadRes.data;
    if (dedupeRes && dedupeRes.data) return dedupeRes.data;
...
//    // if (["User Profile Rejected on System", "User Not Rejected on System"].includes(dedupeRes.data.message)) {
//    //    return "Duplicate";
//    //}
//
//    console.log("lead API sending");
//    const leadReq = {
//        email_id: lead.email,
//        mobile_no: lead.phone,
//        full_name: lead.firstName + " " + lead.lastName,
...
//    console.log("MPKT API Responses:", {
//        dedupeResponse: dedupeRes.data,
//        leadResponse: leadRes.data,
//        statusResponse: statusRes.data,
//    });
//
...
async function fatakPayInject(lead) {
    const fatakpayUrl = `https://credmantra.com/api/v1/partner-api/fatakpay/eligibility`;
    ...
    try {
        ...
        console.error(`FATAK Error processing lead ${lead.phone}`);
        return null;
    }
}
